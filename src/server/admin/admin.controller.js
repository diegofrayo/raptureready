...
import { getHashPassword } from '../user/user.helpers';
...
router.post('/update-user', passport.authenticate("jwt", { session: false }), isAdmin, function (req, res) {
...
router.get('/list', passport.authenticate("jwt", { session: false }), isAdmin, function (req, res) {
...
router.post('/change-password', passport.authenticate("jwt", { session: false }), isAdmin, passport.authenticate("jwt", { session: false }), (req, res) => {
  req.checkBody('password',  'Please provide a valid `password.`').notEmpty();
...
router.post('/create-user', passport.authenticate("jwt", { session: false }), isAdmin, function (req, res) {
...
router.get('/delete', passport.authenticate("jwt", { session: false }), isAdmin, function (req, res) {
  var id = req.query.id;
  ...
  req.getValidationResult()
    .then(function(result) {
      ...
      return User.remove({_id: req.query.id})
    })
    .then(restaurant => res.json(Object.assign({success: true, message: 'The account was successfully deleted.'})))
    .catch((error) => {
      res.json({success: false, error: {message: error.message}})
    });
});
